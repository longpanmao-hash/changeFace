...
function solveLinearSystem6x6(A: number[][], b: number[]): number[] {
  const n = 6;
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    if (Math.abs(M[pivot][i]) < 1e-10) throw new Error("Singular system while fitting affine matrix");
    if (pivot !== i) {
      const t = M[i];
      M[i] = M[pivot];
      M[pivot] = t;
...
function computeAffineLeastSquares(src: Point[], dst: Point[]): AffineMatrix {
  ...
  const addRow = (row: number[], v: number): void => {
    for (let i = 0; i < 6; i += 1) {
      ATb[i] += row[i] * v;
...
function getLandmarkPoints(landmarks: faceapi.FaceLandmarks68): Point[] {
  return landmarks.positions.map((p) => ({ x: p.x, y: p.y }));
}

function getForeheadPoints(landmarks: faceapi.FaceLandmarks68): Point[] {
...
function getFacePolygon(landmarks: faceapi.FaceLandmarks68): Point[] {
  const jaw = landmarks.getJawOutline().map((p) => ({ x: p.x, y: p.y }));
  const forehead = getForeheadPoints(landmarks);
  return [...jaw, ...forehead];
}

function getFeaturePolygons(landmarks: faceapi.FaceLandmarks68): Point[][] {
...
function getAugmentedMesh(landmarks: faceapi.FaceLandmarks68): Point[] {
  return [...getLandmarkPoints(landmarks), ...getForeheadPoints(landmarks)];
}

function computeBounds(points: Point[], width: number, height: number, padding = 20): Rect {
...
function pointInPolygon(p: Point, poly: Point[]): boolean {
  ...
  for (let i = 0, j = poly.length - 1; i < poly.length; j = i++) {
    ...
    const pj = poly[j];
    const intersect =
      pi.y > p.y !== pj.y > p.y && p.x < ((pj.x - pi.x) * (p.y - pi.y)) / (pj.y - pi.y + 1e-8) + pi.x;
...
function buildSoftMask(width: number, height: number, polygon: Point[], feather = 16): Canvas {
...
function buildFeatureMask(width: number, height: number, polygons: Point[]): Canvas {
  const mask = createCanvas(width, height);
  const ctx = mask.getContext("2d");
  ctx.fillStyle = "rgba(0,0,0,0)";
  ctx.fillRect(0, 0, width, height);
  ...
  for (let i = 1; i < polygons.length; i += 1) ctx.lineTo(polygons[i].x, polygons[i].y);
  ctx.closePath();
  ctx.fill();
  return mask;
}

function drawWarpedByTriangles(sourceCanvas: Canvas, srcPts: Point[], dstPts: Point[], facePoly: Point[], width: number, height: number): Canvas {
  const warped = createCanvas(width, height);
  const wctx = warped.getContext("2d");
  const delaunay = Delaunator.from(dstPts.map((p) => [p.x, p.y]));
  ...
  for (let i = 0; i < t.length; i += 3) {
    ...
    if (!pointInPolygon(centroid, facePoly)) continue;
...
function blendFace(
  sourceCanvas: Canvas,
  sourceDetection: faceapi.WithFaceLandmarks<{ detection: faceapi.FaceDetection }, faceapi.FaceLandmarks68>,
  targetCanvas: Canvas,
  targetDetection: faceapi.WithFaceLandmarks<{ detection: faceapi.FaceDetection }, faceapi.FaceLandmarks68>,
  strength: number
): Canvas {
  ...
  const featureMaskCanvas = createCanvas(width, height);
  ...
  if (srcCount === 0 || tarCount === 0) return result;
...
async function loadModels(modelDir: string): Promise<void> {
...
async function detectFace(imagePath: string) {
  ...
  const detection = await faceapi
    .detectSingleFace(canvas as unknown as faceapi.TNetInput, new faceapi.TinyFaceDetectorOptions())
    .withFaceLandmarks(true);
  if (!detection) throw new Error(`No face detected: ${imagePath}`);
...
async function main(): Promise<void> {
  ...
  program
    ...
    .option("-m, --models <path>", "models directory", path.resolve(process.cwd(), "models"));
  ...
  const [argSource, argTarget, argOutput, argStrength] = program.args as [
    string | undefined,
    string | undefined,
    string | undefined,
    string | undefined
  ...
  if (!sourceInput || !targetInput || !outputInput) throw new Error("Missing source/target/output");
  ...
  const modelDir = path.resolve(options.models);
  ...
  if (Number.isNaN(strength)) throw new Error("Invalid --strength value");
